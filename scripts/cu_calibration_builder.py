...
def eq7_decompose(registry, pairs=None):
    ...
    for cell, branches in pairs.items():
        ...
        for br, (ida, idb) in branches.items():
            ...
            # Built as plain locals rather than inline in the f-string: a multi-line
            # expression inside an f-string is PEP 701, i.e. Python 3.12+, and this file
            # must import on the 3.10 the README promises.
            sys_txt = "-" if noise_sys is None else f"{noise_sys:.1f}"
            status = ("RESOLVED" if resolved else
                      "NOT resolved (reduction falls back to closest-length)")
            print(f"  {cell}/{br}: ΔL'' variation std={signal:.1f}, noise "
                  f"stat={noise_stat:.1f} sys={sys_txt} [1e-6] -> {status}")
    registry["eq7"] = eq7
